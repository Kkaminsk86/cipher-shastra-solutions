...
contract Gambler is ERC721("Gambler", "$$$") {
    ...
    function buyChips(uint256 numChips) external {
        ...
        require(
            numChips <= MAX_CHIP_REQUEST,
            "MAX_CHIP_REQUEST per transaction exceeded"
        );
    ...
    function getVerified() external payable {
        ...
        require(
            userBalance > 15,
            "You could have read the code first, instead of blindly buying NFTs"
        );
        require(
            msg.value == userBalance.mul(CHIP_PRICE),
            "Need Chips? Pay Money. Strategy 101"
        );
