...
contract ThirtyFive {
    ...
    bytes32 public constant SIGNING_TYPEHASH =
        keccak256("SIGNING(uint16 nonce,uint256 expiry)");
    ...
    constructor(string memory _name, string memory _version) {
        ...
        DOMAIN_SEPARATOR = keccak256(
            abi.encode(DOMAIN_TYPEHASH, name_, version_, chainId, address(this))
        );
    }
    ...
    function signItLikeYouMeanIt(
        ...
        bytes32 structHash = keccak256(
            abi.encode(SIGNING_TYPEHASH, nonce, deadline)
        );
        ...
        require(
            signer == msg.sender,
            "Only Self Signed Signatures are allowed"
        );
